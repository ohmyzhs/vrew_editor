...
6. 공통 파일 첫 클립의 실제 자막 글꼴·크기·색·외곽선과
   `yAlign`, `yOffset`, `xOffset`, 회전, 폭, 배율을 참고하되,
   최종 영상09에서 확정한 을지로체 170 서식을 모든 자막에 적용합니다.
   자막 문구 자체는 변경하지 않습니다.
7. 구독 클립 직전, 두 번째 구독 클립 끝, 아웃트로 직전의 본편 마지막
   클립에 1초 디졸브 전환을 적용합니다.
...
Windows 사이드카의 JSON 출력은 시스템 코드페이지와 무관한 ASCII
이스케이프 형식으로 전달하므로 한글 파일·폴더 경로를 안전하게 처리합니다.
0.2.1부터 편집 엔진은 Windows의 숨김 프로세스로 실행되어 빈 콘솔 창이
함께 뜨지 않습니다. 엔진의 진행 결과와 오류 출력은 앱 화면에서 그대로
확인할 수 있습니다.
Windows 편집 엔진은 `PYTHONUTF8=1`, `PYTHONIOENCODING=utf-8` 환경으로
실행됩니다. `ffprobe`, PowerShell 파일 선택창 등 외부 프로그램 출력도
운영체제 기본 코드페이지를 사용하지 않고 UTF-8 바이트로 직접 처리합니다.
